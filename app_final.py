...
def register():
    ...
    pwd   = d.get("password","")
    role  = d.get("role","user").lower()
    if role not in ("admin","student","user"): role = "user"
...
def login():
    ...
    token = create_access_token(identity=email)
    return jsonify({"token": token, "name": user["name"], "email": email, "role": user.get("role","user")}), 200
...
def me():
    ...
    user  = get_user_by_email(email)
    return jsonify({"email": email, "name": user["name"] if user else "", "role": user.get("role","user") if user else "user"}), 200
